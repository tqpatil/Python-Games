...
def calculator(gui):   
    ...
    b_pow=addButton(gui,entrybox,'^')#additional buttons start here
    b_dec=addButton(gui,entrybox,'.')
    b_par=addButton(gui,entrybox,"(")
    b_cpar=addButton(gui,entrybox,")")

    # add buttons to the grid
    buttons =[ b7,    b8, b9,    b_clr, 
               b4,    b5, b6,    b_sub, 
               b1,    b2, b3,    b_add, 
               b_mult,b0, b_div, b_eq ,b_pow,b_dec,b_par,b_cpar]
    k = 4         
    for i in range(k):
        for j in range(k):
            buttons[i*k+j].grid(row=i+1, column=j, columnspan=1)
    buttons[16].grid(row=4,column=5,columnspan=1)
    buttons[17].grid(row=3,column=5,columnspan=1)
    buttons[18].grid(row=2,column=5,columnspan=1)
    buttons[19].grid(row=1,column=5,columnspan=1)
